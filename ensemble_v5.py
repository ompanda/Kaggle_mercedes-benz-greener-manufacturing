...
results = auto_classifier.predict(finaltestset)

'''R2 Score on the entire Train data when averaging'''

print('R2 score on train data:')
model_r2_score = r2_score(y_train, results * 0.2855 + model.predict(dtrain) * 0.7145)
print(model_r2_score)

'''Average the prediction test data  of both models then save it on a csv file'''

sub = pd.DataFrame()
sub['ID'] = id_test
sub['y'] = y_pred * 0.75 + results * 0.25
sub.to_csv(base_output_path+'ensemble_v5_r2_score_{}.csv'.format(model_r2_score), index=False)
