...
train_scaled = min_max_scaler.fit_transform(train_reduced)
test_scaled = min_max_scaler.fit_transform(test_reduced)
...
def model():
    model = Sequential()
    # Input layer with dimension input_dims and hidden layer i with input_dims*2 neurons.
    model.add(Dense(input_dims * 2, input_dim=input_dims, activation='relu'))
    # Dropout of 20% of the neurons and activation layer.
    model.add(Dropout(.2))
    model.add(Activation("linear"))
    # Hidden layer j with (input_dims*2)//2 neurons plus activation layer.
    model.add(Dense((input_dims * 2) // 2, activation='relu'))
    model.add(Activation("linear"))
    # Hidden layer k with (input_dims*2)//2 neurons.
    model.add(Dense((input_dims * 2) // 2, activation='relu'))
    # Output Layer.
    model.add(Dense(1))
...
for fold, (train_index, test_index) in enumerate(kf.split(X)):
    ...
    pred = history.predict(X_val)
    predictions[:, fold] = pred

    score += history.best_score

    print('Fold %d: Score %f'%(fold, history.best_score))
...
test_r2_score = r2_score(y_val, estimator.predict(X_val))
print('R^2 val: {}'.format(test_r2_score))  # R^2 val
pass
...
output.to_csv(base_output_path+'keras-r2-{}.csv'.format(test_r2_score), index=False)