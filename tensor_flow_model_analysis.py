...
train_data = np.concatenate((categoricals_train, train_df[binaries].values), axis=1)
test_input = np.concatenate((categoricals_test, test_df[binaries].values), axis=1)

#apply dimension reduction
train_df,test_df = dimension_reduction(train_df,test_df)
...
if SUBMIT:
    ...
    predictions.to_csv(base_output_path+"tf_nn_{}.csv".format(final_r_squared), index=False, header=True)